...
export default async function Mujer() {
  const user = await getSession();
  const distributorPrice = user?.role === "company";
  const productos = await getProducts("mujer");
  return (
    <main className="min-h-screen bg-white text-gray-900">
      <Navbar />
      <section className="relative bg-black text-white">
        <div className="absolute inset-0"><Image src="/colecciones/mujer.jpg" alt="Colección Mujer" fill sizes="100vw" style={{ objectFit: "center", opacity: 0.45 }} /></div>
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-32"><h1 className="text-5xl font-extrabold">Colección Mujer</h1><p className="mt-4 text-gray-200 max-w-xl">Fuerza, estilo y rendimiento para cada entrenamiento.</p></div>
      </section>
