...
export default async function Calzado() {
  const user = await getSession();
  const distributorPrice = user?.role === "company";
  const productos = await getProducts("calzado");
  return (
    <main className="min-h-screen bg-white text-gray-900">
      <Navbar />
      <section className="relative bg-black text-white">
        <div className="absolute inset-0"><Image src="/colecciones/calzado.jpg" alt="Calzado" fill sizes="100vw" style={{ objectFit: "center", opacity: 0.45 }} /></div>
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-32"><h1 className="text-5xl font-extrabold">Calzado</h1><p className="mt-4 text-gray-200 max-w-xl">Rendimiento, confort y diseño para cada paso.</p></div>
      </section>
